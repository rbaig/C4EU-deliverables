...
\fancyhead[R]{ % right
D7.2.1 Report on Activities to Enlist BuB Organizations -- a
}
\pagestyle{fancy}
...
Work Package 7, \emph{Bottom-up-Broadband for Europe}, of Commons4Europe project aims to support the creation of a pan European organization that could provide structure and support to the existing Bottom-up-Broadband initiatives in Europe from either public organizations or emergent from citizen activism. The present document is the first report on the achievements of task T7.2, \emph{Enlisting BuB organizations}, task aim at creating awareness and enlisting the existing BuB organizations to the project. Its success will materialize with the constitution of a second layer organisation aimed at gathering BuB organisations.

\vspace*{\fill}
\end{abstract}

%\renewcommand{\abstractname}{Deliverables D7.2.1 and D7.3.1 merged}
%\begin{abstract}
%%\boldmath
%Due to the fact that tasks T7.2, \emph{Enlisting BuB organizations}, and T7.3, \emph{Building Support for BuB4Europe} are strongly related to each other and that they involve relative small amount of man power (2 and 4 respectively for the first WP year and 2 and 5 for the second -the last), the lead beneficiary of them (guifi.net) has decided to merge their reports, D7.2.1, \emph{Report on activities to enlist BuB organizations - a} and D7.3.1, \emph{Report on Building Support for BuB4Europe - b }, into single present document. This decision has been approved by the project leader (ESADE) and is totally aligned with the recommendation of optimising the paperwork efforts made during the first review process.
%
%\vspace*{\fill}
%\end{abstract}
\renewcommand{\abstractname}{Relationship with D7.3.1}
\begin{abstract}
%\boldmath
In some situations the strong relationship between T7.2, \emph{Enlisting BuB organizations}, and T7.3, \emph{Building Support for BuB4Europe}, makes difficult to clearly assign an action to one task or to the other. The same happens with their deliverables, D7.2.1, \emph{Report on activities to enlist BuB organizations - a} and D7.3.1, \emph{Report on Building Support for BuB4Europe - b }. As a consequence, it is recommended to read both deliverables to have a complete view of the job done.
...
\section{Plans for the next year}
\label{sec:L2org}
\input{sect3/future}
